...
// Methods from ::vendor::libra::power::V1_0::IPower follow.
Return<SupportedProfiles> Power::getSupportedProfilesCount() {
    int count = 0;
    if (mModule->getFeature)
        count = mModule->getFeature(mModule,
                static_cast<feature_t>(POWER_FEATURE_SUPPORTED_PROFILES));

    SupportedProfiles profilesCount;
    switch (count) {
        case 3:
            profilesCount = SupportedProfiles::BASE;
            break;
        case 5:
            profilesCount = SupportedProfiles::EXTENDED;
        default:
            profilesCount = SupportedProfiles::UNSUPPORTED;
    }
    return profilesCount;
}

Return<void> Power::setProfile(Profile profile) {
    if (mModule->powerHint) {
            mModule->powerHint(mModule,
                    static_cast<power_hint_t>(POWER_HINT_SET_PROFILE), &profile);
    }
    return Void();
}

Return<void> Power::boost(uint32_t duration) {
    if (mModule->powerHint) {
            mModule->powerHint(mModule,
                    static_cast<power_hint_t>(POWER_HINT_CPU_BOOST), &duration);
    }
    return Void();
}
